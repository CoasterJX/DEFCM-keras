...
def load_pendigits(data_path='../data/raw/pendigits'):
    import os
...
def load_usps(data_path='../data/raw/usps'):
    import os
...
def load_reuters(data_path='../data/raw/reuters'):
    import os
...
def load_cifar10(data_path='../data/raw/cifar10'):
...
def load_stl(data_path='../data/raw/stl'):
    import os
    ...
    if os.path.exists(data_path + '/stl_features.npy'):
        ...
        y = np.concatenate((y1, y2))
        print(np.load(data_path + '/stl_features.npy').shape)
    ...
    for i in range(13):
        if features is None:
            features = extract_vgg16_features(x[:batch_size])
        else:
            features = np.append(features, extract_vgg16_features(x[:batch_size]), axis=0)
        x = x[batch_size:]
        print(f"Preprocessing batch {i+1} finished, feature shape={features.shape}, remaining={x.shape}")
    ...
    # save features
    np.save(data_path + '/stl_features_less.npy', features)
    print('features saved to ' + data_path + '/stl_features_less.npy')
